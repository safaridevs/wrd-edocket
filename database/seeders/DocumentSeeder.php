...
class DocumentSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $cases = CaseModel::whereIn('case_no', ['25-001', '25-002', '25-003'])->get();

        foreach ($cases as $case) {
            foreach ($this->documentsForCase($case, $aluClerk, $huAdmin) as $document) {
    ...
    private function documentsForCase(CaseModel $case, User $aluClerk, User $huAdmin): array
    {
        $active = $case->status === 'active';
        $stampText = $active
            ? "ELECTRONICALLY FILED\n" . now()->format('D m/d/Y @ g:i A') . "\nOSE HEARING UNIT / {$huAdmin->initials}"
            : null;
        ...
        return [
            [
                ...
                'stamped_at' => $active ? now()->subDays(3) : null,
                'approved' => $active,
                'approved_by_user_id' => $active ? $huAdmin->id : null,
                'approved_at' => $active ? now()->subDays(3) : null,
                'pleading_type' => 'none',
            ],
            [
                'case_id' => $case->id,
            ...
            [
                ...
                'approved_by_user_id' => $active ? $huAdmin->id : null,
                'approved_at' => $active ? now()->subDays(3) : null,
                'pleading_type' => 'request_to_docket',
            ],
        ];
    }
}
